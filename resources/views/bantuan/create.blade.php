...
<div class="card shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="card-title mb-0 fw-bold">
            <i class="fas fa-hand-holding-heart me-2" style="color: #1a472a;"></i>
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('bantuan.store') }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           maxlength="16"
                           value="{{ old('nik') }}" 
                           placeholder="Ketik 16 Digit NIK Penerima..." 
                           required 
                           autofocus>
                    <div id="nik_status_badge" class="info-badge-autofill mt-1">
                        <i class="fas fa-check-circle me-1"></i> Data ditemukan di database warga! Form terisi otomatis.
                    </div>
                    @error('nik')
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           name="alamat" 
                           value="{{ old('alamat') }}" 
                           placeholder="Contoh: DUSUN VI SIDOMULYO B">
                    @error('alamat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6 d-none" id="program_lainnya_wrapper">
                    ...
                    <input type="text" class="form-control" id="program_bantuan_lainnya" name="program_bantuan_lainnya" 
                           placeholder="Misal: Bantuan Sembako Desa" value="{{ old('program_bantuan_lainnya') }}">
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" class="form-control @error('keterangan') is-invalid @enderror" 
                           name="keterangan" value="{{ old('keterangan') }}" placeholder="Contoh: Tahap I 2026, Nominal Rp 300.000, Beras 10kg, dll">
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mt-4 pt-2 d-flex gap-3 flex-wrap">
                ...
                <button type="submit" class="btn-action btn-simpan">
                    <i class="fas fa-save"></i> Simpan Data Penerima Bantuan
                </button>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // ===== PROGRAM BANTUAN LAINNYA TOGGLE =====
    ...
    // ===== AUTO-LOOKUP PENDUDUK BERDASARKAN NIK =====
    ...
    const badgeAutofill = document.getElementById('nik_status_badge');

    let timeout = null;
    nikInput.addEventListener('input', function() {
        const nikVal = this.value.trim();
        if (nikVal.length >= 15) {
            clearTimeout(timeout);
            timeout = setTimeout(() => {
                fetch(`{{ route('bantuan.search-penduduk') }}?nik=${encodeURIComponent(nikVal)}`)
                    .then(res => res.json())
                    .then(data => {
                        if (data && data.nama) {
                            if (namaInput) namaInput.value = data.nama;
                            if (noKkInput && data.no_kk) noKkInput.value = data.no_kk;
                            if (tglLahirInput && data.tanggal_lahir) {
                                const d = new Date(data.tanggal_lahir);
                                if (!isNaN(d.getTime())) {
                                    tglLahirInput.value = d.toISOString().split('T')[0];
                                }
                            }
                            if (jkInput && data.jenis_kelamin) jkInput.value = data.jenis_kelamin;
                            if (pekerjaanInput && data.pekerjaan) pekerjaanInput.value = data.pekerjaan;
                            if (alamatInput && data.alamat) alamatInput.value = data.alamat;

                            badgeAutofill.style.display = 'block';
                        } else {
