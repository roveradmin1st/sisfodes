...
@section('page-title', 'Dashboard Kaur Umum')
    ...
    <div class="col-md-3 animate-on-scroll" style="animation-delay: 0.3s;">
        <div class="stat-card" style="border-left-color: #ffc107;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <p class="stat-label mb-1">
                        <i class="fas fa-clock me-1" style="color: #856404;"></i>
                        Menunggu Verifikasi
                    </p>
                    <h3 class="stat-number" style="background: linear-gradient(135deg, #ffc107, #e0a800); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-block;">{{ $menungguVerifikasi }}</h3>
                </div>
...
<!-- ============================================================ -->
<!-- GRAFIK PENDUDUK + PENGAJUAN TERBARU (BERSAMPINGAN)          -->
<!-- ============================================================ -->
<div class="row g-4 mb-4">
    <!-- Grafik Penduduk -->
    <div class="col-md-8 animate-on-scroll" style="animation-delay: 0.5s;">
    ...
    <div class="col-md-4 animate-on-scroll" style="animation-delay: 0.6s;">
        <div class="card shadow-sm">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0 fw-bold">
                    <i class="fas fa-file-alt me-2 text-primary"></i>Pengajuan Terbaru
                    <span class="badge" style="background: linear-gradient(135deg, #0d6efd, #0a58ca); color: white; border-radius: 20px; padding: 4px 12px; font-size: 0.7rem; margin-left: 6px;">{{ $pengajuanTerbaru->count() }}</span>
                </h5>
                <a href="{{ route('surat.permohonan.index') }}" class="btn btn-sm btn-link text-primary p-0" style="transition: all 0.3s ease;">
            ...
            <div class="card-body" style="max-height: 340px; overflow-y: auto;">
                @if($pengajuanTerbaru->count() > 0)
                    @foreach($pengajuanTerbaru as $item)
                        <div class="pengajuan-item d-flex align-items-start">
                            <div class="flex-shrink-0">
                                <span class="badge-icon-sm badge-{{ $item->status_permohonan }}">
                                    <i class="fas fa-{{ $item->status_permohonan == 'menunggu' ? 'clock' : ($item->status_permohonan == 'diproses' ? 'spinner' : ($item->status_permohonan == 'selesai' ? 'check' : 'times')) }}"></i>
                                </span>
                            </div>
                            <div class="flex-grow-1 ms-2">
                                <p class="mb-0 small" style="line-height: 1.3;">
                                    <strong style="color: #1a472a;">{{ $item->penduduk->nama ?? '-' }}</strong>
                                    <br>
                                    <span class="text-muted" style="font-size: 0.75rem;">{{ $item->jenisSurat->nama_surat ?? '-' }}</span>
                                </p>
                                <small class="text-muted" style="font-size: 0.65rem;">
                                    <i class="far fa-calendar-alt me-1"></i>{{ $item->tanggal_pengajuan->format('d/m/Y') }}
                                </small>
                            </div>
                            <div class="ms-2">
                                <span class="badge-status-sm badge-{{ $item->status_permohonan }}-sm">
                ...
                @else
                    <div class="text-center py-4 text-muted">
                        ...
                        <p class="small">Belum ada pengajuan</p>
                    </div>
                @endif
            </div>
        </div>
